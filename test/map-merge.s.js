...
describe('map-merge.s', function () {
  it('should merge stream with map function', function (done) {
    ...
    var oneToFive = numbers({ from: 1, to: 5,  objectMode: true });
    var sixToTen  = numbers({ from: 6, to: 10, objectMode: true });
  ...
  it('should drop packets from longer stream', function (done) {
    var results = [];
    var sum = merge(function (first, second, done) {
      done(null, first + second);
    });
    var oneToFive = numbers({ from: 1, to: 5, objectMode: true });
    var sixToTen  = numbers({ from: 6, to: 8, objectMode: true });
    sum.on('readable', function () {
      var item;
      while ((item = sum.read()) !== null) {
        results.push(item);
